...
export default function DairyRouteSheets() {
    ...
    () =>
      shipments.flatMap((shipment) =>
        (shipment.stops || []).map((stop) => ({
          tripId: shipment.tripId || shipment.id,
  ...
  return (
    <div className="p-6 text-[#1F2A44]">
      <h1 className="text-2xl font-semibold text-[#1E4B6B]">Trip Route Sheet</h1>
      <p className="mt-1 text-sm text-[#5B6B7F]">
        Admin assigns route plus tanker to create trips. Dairy verifies each trip from BMC stop data.
      </p>
          ...
          <table className="w-full min-w-[920px] text-left text-sm">
            <thead className="bg-[#EEF4FF] text-[#1E4B6B]">
              <tr>
                ...
                <th className="px-4 py-3">Society Count (via BMC)</th>
            ...
            <tbody>
              {routeSheetRows.map((row, index) => (
                <tr key={`${row.bmc}-${index}`} className="border-t border-[#E6EDF7]">
                  <td className="px-4 py-3 font-semibold text-[#1E4B6B]">{row.tripId}</td>

...
function Card({ label, value }) {
  const toneMap = {
    ...
    "Trips Received": "border-[#CFE0FF] bg-[#EEF4FF]",
...
export default function DairyDashboard() {
  ...
  const dashboardCards = useMemo(
    () => [
      ...
      { label: "Trips Received", value: String(metrics.tripCount) },
  ...
  return (
    <div className="p-6 text-[#1F2A44]">
      ...
      <p className="mt-1 text-sm text-[#5B6B7F]">Trip-level verification and transport accountability for the current shift.</p>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-3">
        <section className="rounded-xl border border-[#D7E4FF] bg-white p-5 shadow-[0_4px_12px_rgba(15,41,74,0.08)]">
          <h2 className="text-lg font-semibold text-[#1E4B6B]">Operational Model</h2>
          <div className="mt-3 space-y-2 text-sm text-[#334155]">
            <p>Society to BMC collection happens before tanker movement.</p>
            <p>Tanker trips collect from BMC stops, not from societies directly.</p>
            <p>Dairy verifies each trip: expected, received, quality, and shortage.</p>
            <p>Approved trips move to accounts with deductions when required.</p>
          </div>
        </section>
